              ...
              <div className="flex flex-wrap gap-4 lg:justify-end">
                {brand.url && brand.url !== "#" && (
                  <Button asChild variant="outline" className="rounded-none border-white/20 hover:bg-white hover:text-black font-black uppercase italic">
                    <a href={brand.url} target="_blank" rel="noopener noreferrer">Official Specs <ExternalLink className="w-4 h-4 ml-2" /></a>
                  </Button>
                )}
          ...
          <div className="max-w-7xl mx-auto px-6">
            <div className="grid lg:grid-cols-3 gap-16">
              <div className="lg:col-span-2 prose prose-slate max-w-none">
                <h2 className="text-4xl font-black uppercase italic tracking-tighter mb-8">Engineering the <span className="text-red-600">Arizona Standard</span></h2>
                <div className="text-lg font-medium text-slate-700 space-y-6">
                  <p>Arizona House of Film is a factory-certified installer of <strong>{brand.name}</strong> window films. In our desert climate, where surface temperatures can exceed 160°F, we choose {brand.name} for its industry-leading thermal stability and molecular-level UV inhibitors.</p>
                  <p>Whether you are targeting massive HVAC savings or protecting high-end interiors from solar degradation, {brand.name} provides the nanotechnology required for long-term performance in Phoenix and Scottsdale.</p>
                </div>
              ...
              <aside className="space-y-8">
                <div className="bg-slate-950 text-white p-8 border-t-8 border-red-600">
                  <h3 className="font-black uppercase italic tracking-tighter text-2xl mb-8 border-b border-white/10 pb-4">Performance <span className="text-red-600 text-sm block">Metrics</span></h3>
        ...
        {faqSchema?.mainEntity && (
          <section className="py-24 border-t border-white/5 bg-slate-950">
            <div className="max-w-4xl mx-auto px-6">
              <h2 className="text-3xl font-black uppercase italic tracking-tighter mb-12 text-center underline decoration-red-600 underline-offset-8">Technical FAQ</h2>
              <div className="space-y-4">
                {faqSchema.mainEntity.map((faq, index) => (
                  <motion.div key={index} className="bg-slate-900/50 border border-white/10 p-6 hover:border-red-600/50 transition-colors" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }}>
        ...
        {relatedBlogPosts.length > 0 && (
          <section className="py-20 bg-slate-50 text-slate-950">
            <div className="max-w-7xl mx-auto px-6 text-center">
              <h3 className="text-2xl font-black uppercase italic tracking-tighter mb-8">Field Reports & Insights</h3>
              <div className="grid md:grid-cols-3 gap-6">
                {relatedBlogPosts.map(post => (
                  <Link key={post.slug} to={`/blog/${post.slug}`} className="bg-white p-6 border border-slate-200 hover:shadow-xl transition-shadow text-left">
                    ...
                    <span className="text-red-600 font-bold text-xs uppercase tracking-widest">Read Article →</span>
                  </Link>
                ))}
              </div>
            </div>
