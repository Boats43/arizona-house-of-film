...
const SupportingPage = () => {
  ...
  const pageData = supportingPagesData[pageKey];

  if (!pageData) {
    // Fallback for single-slug URLs if needed
    const singleSlugData = supportingPagesData[slug];
    if (singleSlugData) {
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 2, "name": hero.parent, "item": `https://arizonahouseoffilm.com${hero.parentLink}` },
    ...
    <>
      ...
      <div className="bg-white">
        <header className={`relative py-20 text-white ${hero.gradient}`}>
          {hero.imageUrl && <img alt={hero.imageAlt} className="absolute inset-0 w-full h-full object-cover" src="https://images.unsplash.com/photo-1649215636705-1084bd6df97a" />}
          <div className="absolute inset-0 bg-black/40" />
          <div className="relative z-10 max-w-4xl mx-auto px-4 text-center">
             <div className="mb-4">
              <Link to={hero.parentLink} className="text-white hover:underline flex items-center justify-center text-sm">
        ...
        <section className="py-20 bg-blue-600 text-white">
          <div className="max-w-4xl mx-auto px-4 text-center">
            <h2 className="text-3xl font-bold mb-4">Ready to Enhance Your Property?</h2>
            <p className="text-lg text-blue-100 mb-8">
              Contact Arizona House of Film today for a free, no-obligation estimate. Our experts are ready to help you choose the perfect window film solution.
            </p>
...
// Helper component for backwards compatibility with old single-slug structure
const SingleSlugRedirect = ({ data }) => {
    return <Navigate to={data.canonicalUrl.replace('https://arizonahouseoffilm.com', '')} replace />;
};
